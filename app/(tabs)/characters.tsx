    ...
    return (
        <SafeAreaView className="flex-1 bg-[#FFF5F7]">
            <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 32, paddingBottom: 120 }}>
                <Text className="text-3xl font-fredoka-bold text-[#FF69B4] mb-8">{t('tabs.characters')}</Text>
...
const CharacterCard = ({ name, image, selected, locked }: any) => (
    ...
    >
        ...
        <Text className="text-center font-fredoka-bold text-[#FF69B4] mt-2">{name}</Text>
        {locked && (
            <Text className="text-center text-[#FFB6C1] text-[10px] uppercase font-fredoka-bold mt-1">Locked</Text>
        )}
    </TouchableOpacity>
);
